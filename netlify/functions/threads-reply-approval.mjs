...
export default async function(req){
  ...
  if(body.action==='prepare'){
    ...
    for(const item of remaining){item.status='approved';item.approvedAt=new Date().toISOString()}
  }else if(body.action==='reject'){
    if(remaining.some(x=>!['pending','awaiting_approval','rejected'].includes(x.status)))
      return Response.json({error:'제외할 수 없는 상태입니다.'},{status:409});
    for(const item of remaining){item.status='rejected';item.rejectedAt=new Date().toISOString()}
