...
function sourceDraft(items){
  const topics={
    route:['하카타','텐진','캐널시티','오호리공원','모모치'],
    food:['라멘','모츠나베','멘타이코','야키니쿠','텐동'],
    nearby:['다자이후','유후인']
  };
  const found={route:[],food:[],nearby:[]};
  for(const [group,words] of Object.entries(topics)){
    for(const word of words){
      const index=items.findIndex(x=>(x.title+' '+x.description).includes(word));
      if(index>=0)found[group].push({term:word,source:index+1});
    }
  }
  const line=(group,max)=>found[group].slice(0,max).map(x=>x.term+' ['+x.source+']').join(' · ');
  const lines=['🔎 최근 여행 후기 검색 요약으로 만든 후쿠오카 초안'];
  if(found.route.length)lines.push('🗓️ 동선: '+line('route',3)+' 일정을 묶을 때 위치와 이동시간을 확인해보세요.');
  if(found.food.length)lines.push('🍜 먹거리: '+line('food',3)+' 관련 후기를 살펴보고 영업시간을 확인하세요.');
  if(found.nearby.length)lines.push('🚃 근교: '+line('nearby',2)+' 왕복 교통편과 소요시간을 먼저 확인하세요.');
  if(lines.length===1)lines.push('검색 요약에서 구체적인 장소·음식 정보를 확인하지 못했습니다.');
  lines.push('※ [번호]는 아래 검색 후보입니다. 블로그 본문은 아직 확인되지 않았습니다.');
  return {text:lines.join('\n'),matches:found,sourceCount:items.length,verifiedFullText:false};
}
